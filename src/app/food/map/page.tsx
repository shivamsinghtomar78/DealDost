...
export default function FoodMapPage() {
    ...
    useEffect(() => {
        ...
        const loadSpots = async () => {
            setLoading(true);
            const data = await fetchFoodSpots({ limit: 60, signal: controller.signal });
            if (!active) return;
            setSpots(data);
            setLoading(false);
        };
    ...
    const center = useMemo(() => {
        if (spots.length === 0) {
            return { lat: 28.6500, lng: 77.2100 };
        }
        const avgLat = spots.reduce((sum, spot) => sum + spot.coordinates.lat, 0) / spots.length;
        const avgLng = spots.reduce((sum, spot) => sum + spot.coordinates.lng, 0) / spots.length;
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-6">
            <div className="flex items-center justify-between mb-6 gap-3">
                <div>
                    ...
                    <p className="text-text-muted text-sm mt-1">
                        Discover {spots.length} food spots near you
                    </p>
                </div>
            ...
            {loading ? (
                <div className="h-[500px] rounded-2xl border border-border bg-white dark:bg-[#1e2028] dark:border-[#2a2d34] skeleton-shimmer" />
            ) : spots.length === 0 ? (
                <div className="rounded-2xl border border-border bg-white dark:bg-[#1e2028] dark:border-[#2a2d34] p-8 text-center">
                    <p className="text-lg font-bold text-text-primary dark:text-white">No food spots available</p>
                    <p className="text-sm text-text-secondary mt-1">Add spots from Post New to populate the map.</p>
                </div>
            ) : (
                    ...
                    {view === "split" && (
                        <div className="lg:w-96 space-y-3 max-h-[500px] overflow-y-auto">
                            <div className="flex items-center gap-2 px-1 pb-2 border-b border-border dark:border-[#2a2d34]">
                                ...
                                <span className="text-sm font-semibold text-text-secondary">{spots.length} spots found</span>
                            </div>
                            ...
                            {spots.map((spot) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h3 className="text-sm font-bold text-text-primary dark:text-white truncate group-hover:text-primary transition-colors">{spot.placeName}</h3>
                                        <p className="text-xs text-text-muted mt-0.5">{spot.dishName} · {spot.dishCategory}</p>
                                        <div className="flex items-center gap-2 mt-1.5">
                                            <span className="text-xs font-medium text-green-600 dark:text-green-400">{spot.priceRange}</span>
                                            <span className="text-xs text-text-muted">·</span>
                                            <span className="text-xs text-text-muted flex items-center gap-0.5"><MapPin className="w-3 h-3" /> {spot.area}</span>
                                        </div>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-xs text-text-muted">{spot.upvotes} upvotes</span>
                                            <span className="text-xs text-text-muted">{spot.beenHereCount} visited</span>
                                        </div>
                                    </div>
                                </Link>
                            ))}
